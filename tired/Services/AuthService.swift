...
final class AuthService: NSObject, ObservableObject {
    ...
    deinit {
        firebase.removeAuthStateListener(authHandle)
    }
    ...
    private func message(for error: Error) -> String {
        if let authError = error as? AuthErrorCode {
            ...
            case .userNotFound: return "找不到對應帳號，請檢查電子郵件。"
            case .wrongPassword: return "密碼不正確，請再試一次。"
            case .emailAlreadyInUse: return "此電子郵件已被使用。"
            case .credentialAlreadyInUse: return "此登入方式已綁定其他帳號。"
            case .invalidEmail: return "電子郵件格式不正確。"
            case .tooManyRequests: return "嘗試次數過多，請稍後再試。"
            case .internalError: return "Firebase 設定有誤，請聯絡系統管理員。"
...
private final class UserPreferencesStore {
    ...
    func savePhoneNumber(_ phone: String, for userId: String) {
        var preferences = loadPreferences(for: userId)
        // 預留擴充偏好設定的儲存位置
